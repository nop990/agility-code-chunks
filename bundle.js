...
var languages = ["markup", "css", "clike", "javascript", "bash", "c", "csharp", "cpp", "cobol", "css-extras", "csv", "git", "gradle", "groovy", "http", "java", "json", "json5", "kotlin", "less", "markdown", "powershell", "python", "r", "jsx", "tsx", "sass", "scss", "shell-session", "sql", "typescript", "typoscript", "xml-doc", "yaml"];
setTimeout(function () {
...
function revertDocument() {
  ...
  document.querySelectorAll('pre > code').forEach(function (codeElement) {
    ...
    if (languageClass) {
      var language = languageClass.replace('language-', '');
      if (languages.includes(language)) {
        var formattedCode = preElement.outerHTML;
        if (formattedCode) {
          var lines = formattedCode.split('\n');
          console.log('%cLines', 'color: blue;', lines);
          var revertedCode = ["<p>''' ".concat(language, "</p>")];
          lines.map(function (line) {
            line = line.replace(/<pre[^>]*><code[^>]*>/g, '').replace(/<\/code><\/pre>/g, '').replace(/<span[^>]*>/g, '').replace(/<\/span>/g, '').replace(/<\/p><p><\/p>/g, '</p>');
            revertedCode.push("<p>".concat(line.trim(), "</p>"));
          });
          revertedCode.push("<p>'''</p>");
          console.log('%cReverted Code', 'color: purple;', revertedCode);
          var finalCode = revertedCode.join('');
          console.log('Reverted', finalCode);
          preElement.outerHTML = finalCode;
          document.querySelectorAll('div.toolbar > div.toolbar-item > button').forEach(function (button) {
            button.outerHTML = '';
          });
        }
      }
...
/* PrismJS 1.29.0
https://prismjs.com/download.html#themes=prism-tomorrow&languages=markup+css+clike+javascript+bash+c+csharp+cpp+cobol+css-extras+csv+git+gradle+groovy+http+java+json+json5+kotlin+less+markdown+powershell+python+r+jsx+tsx+sass+scss+shell-session+sql+typescript+typoscript+xml-doc+yaml&plugins=line-numbers+autolinker+highlight-keywords+inline-color+command-line+normalize-whitespace+toolbar+copy-to-clipboard+treeview */
    ...
    for (var i = 0; i < tokens.length; i++) {
      ...
      if (notTagNorBrace || typeof token === 'string') {
        if (openedTags.length > 0 && openedTags[openedTags.length - 1].openedBraces === 0) {
          ...
          var plainText = _stringifyToken(token);
